...
inline block sm4_enc(const block src_block, const u4 rkey[SM4_KEY_SCHEDULE])
{
	u1 src[SM4_BLOCK_SIZE], dst[SM4_BLOCK_SIZE];
	memcpy(src, &src_block, sizeof(block));
	...
	block dst_block;
	memcpy(&dst_block, dst, sizeof(block));
	return dst_block;
}

inline void sm4_enc_blks(block *blks, unsigned int nblks, const u4 key[SM4_KEY_SCHEDULE]) {
	for(int i = 0; i < nblks; i++) {
		blks[i] = sm4_enc(blks[i], key);
	}
}
...
inline void sm4_key_schedule(const block key_block, u4 rkey[SM4_KEY_SCHEDULE])
{
	u4 i = 0;
