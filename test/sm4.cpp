...
void sm4_self_check()
{
	...
	uint8_t key_byte[SM4_KEY_SIZE] = { 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10, };
	block key;
	memcpy(&key, key_byte, sizeof(block));
	...
	block plain_block;
	memcpy(&plain_block, plain, sizeof(block));
	block cipher_block = sm4_enc(plain_block, rkey);
	...
	if (memcmp(&cipher_block, expected, SM4_BLOCK_SIZE) != 0)
	...
	memcpy(cipher, &cipher_block, sizeof(block));
...
void sm4_benchmark()
{
	...
	for (i = 0; i < n; i++)
	{
		cipher_block = sm4_enc(plain_block, rkey);
		// sm4_enc(plain, cipher, rkey);
		// sm4_enc_ex(plain, cipher, rkey);
	}
	t2 = clock();
